    ...
    return (
      <div className="h-[400px] w-full text-neutral flex items-center justify-center">
        <div className="text-center">
          <h2 className="text-2xl font-bold mb-3 bg-gradient-to-r from-primary to-accent text-transparent bg-clip-text">No Budget Yet</h2>
  ...
  return (
    <div className="w-full h-[600px] p-6 flex flex-col">
      <div className="flex flex-col space-y-6">
        <div className="mb-10">
          <h2 className="text-3xl font-extrabold truncate bg-gradient-to-r from-primary to-accent text-transparent bg-clip-text">
            {budget.name || "Current Budget"}
          </h2>
          <p className="text-neutral text-base mt-3">
            {budget.start_date && budget.end_date ? (
              <>Budget Date Range: {formatDate(budget.start_date)} – {formatDate(budget.end_date)}</>
            ) : budget.start_date ? (
          ...
          <p className="text-neutral text-base mt-1">
            Status:{" "}
            <span
              className={budget.is_active ? "text-success font-semibold" : "text-error font-semibold"}
            >
          ...
          <div style={{ width: "280px", height: "250px" }}>
            <CircularProgressbar
              ...
              styles={buildStyles({
                ...
                pathColor: usedPercentage > 100 ? "oklch(0.6000 0.2000 20.00)" : "oklch(0.6178 0.1324 70.26)",
        ...
        <div className="pt-4 flex flex-col items-center space-y-2 pt-18">
          ...
          <button
            className="px-6 py-3 text-sm bg-gradient-to-r from-primary to-accent hover:from-primary/90 hover:to-accent/90 text-light rounded-xl font-semibold shadow-md hover:shadow-lg transition-all duration-200 transform hover:scale-105"
            onClick={onEdit}
          >
            Edit Budget
          </button>
